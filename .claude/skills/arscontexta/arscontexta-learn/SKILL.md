...
- If topic empty: read `arscontexta/self/goals.md` for highest-priority unexplored direction and propose it
...
```
arscontexta/ops/config.yaml             — research tools, depth, pipeline chaining
arscontexta/ops/derivation-manifest.md  — domain vocabulary (inbox folder, reduce skill name)
```
...
- Inbox folder: `arscontexta/inbox/` (could be `journal/`, `encounters/`, etc.)
- Reduce skill name: `/extract` (could be `/surface`, `/break-down`, etc.)
...
**Write to** the domain inbox folder (from derivation-manifest, default `arscontexta/inbox/`). Create folder if missing.
...
If `arscontexta/self/goals.md` exists AND the research uncovered meaningful new directions:
