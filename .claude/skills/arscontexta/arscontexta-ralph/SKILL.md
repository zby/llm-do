...
| create | (inline note creation) | Write the note file |
| enrich | /enrich | Add content to existing note |
| reflect | /connect | Find connections, update indexs |
| reweave | /revisit | Update older notes with new connections |
| verify | /review | Description quality + schema + health checks |
...
1. `arscontexta/ops/queue.yaml`
2. `arscontexta/ops/queue/queue.yaml`
3. `arscontexta/ops/queue/queue.json`
...
tasks:
  - id: source-name
    type: extract
    status: pending
    source: arscontexta/ops/queue/archive/2026-01-30-source/source.md
    file: source-name.md
...
```
Read the task file at arscontexta/ops/queue/{FILE} for context.
...
Create a note for this claim in docs/notes/[claim as sentence].md
...
The task file specifies which existing note to enrich and what to add.
ONE PHASE ONLY. Do NOT run reflect.
```
...
Use dual discovery: index exploration AND semantic search.
Add inline links where genuine connections exist — including sibling claims listed above.
Update relevant index with this note.
...
This is the BACKWARD pass. Find OLDER notes AND sibling claims
that should reference this note but don't.
Add inline links FROM older notes TO this note.
...
note title, path, and claim ID. The lead needs this for cross-connect.
```
