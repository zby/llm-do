...
2. **`arscontexta/ops/config.yaml`** — pipeline chaining mode, automation settings
...
The task stack (`arscontexta/ops/tasks.md`) and the pipeline queue (`arscontexta/ops/queue/queue.yaml` or `arscontexta/ops/queue/queue.json`) serve different purposes:
...
| Pipeline queue | Automated processing state — what the SYSTEM needs to process | Pipeline skills | Automatic: /extract, /ralph, /connect |
...
```bash
# Read arscontexta/ops/tasks.md
cat arscontexta/ops/tasks.md 2>/dev/null
```
...
If `arscontexta/ops/tasks.md` does not exist, note: "No task stack found. Run `/tasks add [description]` to create one."
...
--=={ tasks }==--
  ...
  Task Stack (arscontexta/ops/tasks.md)
  =========================
  Current:
    1. [ ] {task description}
    2. [ ] {task description}
...
**Step 1: Read current arscontexta/ops/tasks.md**
...
Show only the Discoveries section from arscontexta/ops/tasks.md.
...
Discoveries are captured during pipeline work (e.g., /extract notes a connection opportunity, /connect notices a split candidate). They accumulate here until the user decides to convert them to tasks or discard them.

---

## Queue Integration

The task stack (arscontexta/ops/tasks.md) and pipeline queue coexist but serve different audiences:

| Aspect | Task Stack | Pipeline Queue |
|--------|-----------|---------------|
| File | arscontexta/ops/tasks.md | arscontexta/ops/queue/queue.yaml (or .json) |
...
- `/extract` adds tasks to the queue AND notes discoveries in tasks.md
...
- MOC [[topic]] might need splitting (40+ notes observed during /connect)
```
...
If multiple agents modify arscontexta/ops/tasks.md simultaneously, last write wins. The file is small enough that conflicts are unlikely, but if detected, report: "Task stack may have been modified by another session. Please review."
