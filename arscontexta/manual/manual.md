...
| [[configuration]] | arscontexta/ops/config.yaml dimensions, processing depth, feature flags |
...
- **arscontexta/self/** -- Agent identity and persistent memory. Read at every session start. Contains identity.md, methodology.md, goals.md, and memory/.
- **arscontexta/ops/** -- Operational state: config.yaml, processing queue, observations, tensions, session logs.
