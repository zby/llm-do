...
- **16 local skills** -- Installed in `arscontexta/skills/` and `.claude/skills/`. Invoked as `/arscontexta-[name]`. These handle note processing, graph analysis, and system maintenance.
...
|----------|--------|---------|
| Local skills | `/arscontexta-[name] [args]` | `/arscontexta-extract arscontexta/inbox/research.md` |
...
These skills form the core pipeline that source material flows through. Each phase operates on notes in docs/notes/ and advances the queue entry in arscontexta/ops/queue/queue.json.
...
- (empty) -- scan arscontexta/inbox/ for unprocessed items
...
**Arguments:** `[file path]` -- source file to seed (checks arscontexta/inbox/ if not found at given path)

**What it does:**
1. Validates the source file exists
2. Checks for duplicate processing
3. Creates an archive folder structure
4. Moves the source from arscontexta/inbox/ to the archive
5. Creates an extract task in arscontexta/ops/queue/queue.json
...
5. Goals -- active threads from arscontexta/self/goals.md
...
2. Files results to arscontexta/inbox/ with provenance metadata (source_type, research_prompt, generated timestamp)
3. Chains to the processing pipeline based on config chaining mode
4. Updates arscontexta/self/goals.md with new research directions discovered
...
- **Session mining** -- `/arscontexta-remember --mine` -- scan arscontexta/ops/sessions/ for recurring patterns
...
**What it does:** Compares arscontexta/ops/config.yaml against arscontexta/ops/derivation.md to find dimension shifts, then generates a restructuring plan. Never executes without approval.
...
**Purpose:** Initialize the knowledge system for a new project. Creates arscontexta/self/, arscontexta/ops/, arscontexta/templates/, arscontexta/manual/, and configures all dimensions based on a conversation about the project.
...
**Purpose:** Query two knowledge layers -- local methodology (arscontexta/ops/methodology/) and bundled research -- to answer questions about knowledge system design.
