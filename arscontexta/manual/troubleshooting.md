...
# Queue status
cat arscontexta/ops/queue/queue.json 2>/dev/null | python3 -c "
import json, sys
...
ls arscontexta/ops/observations/*.md 2>/dev/null | wc -l
ls arscontexta/ops/tensions/*.md 2>/dev/null | wc -l
```
...
**Prevention:** The extraction skill's skip rate target for llm-do sources is < 10%. If consistently higher, check `processing.extraction.selectivity` in arscontexta/ops/config.yaml.
...
grep 'semantic-search' arscontexta/ops/config.yaml
...
3. **Queue file corruption** -- invalid JSON in arscontexta/ops/queue/queue.json

**Diagnosis:**
```bash
# Check queue file validity
python3 -c "import json; json.load(open('arscontexta/ops/queue/queue.json'))" 2>&1

# Find stuck tasks
python3 -c "
import json
q = json.load(open('arscontexta/ops/queue/queue.json'))
...
3. If JSON corruption: check git history for the last valid version of arscontexta/ops/queue/queue.json
...
**Symptoms:** Reading arscontexta/self/ and running `/arscontexta-next` consumes significant session time.

**Cause:** arscontexta/self/goals.md has accumulated too many threads, or arscontexta/ops/queue/queue.json is very large.

**Fix:**
1. Clean up arscontexta/self/goals.md -- archive completed threads, consolidate related threads
2. Archive completed queue entries (move done tasks out of the active queue file)
3. Keep arscontexta/self/goals.md focused on 3-5 active threads maximum

---

### Skills report "derivation manifest not found"

**Symptoms:** A skill warns about missing arscontexta/ops/derivation-manifest.md and falls back to universal defaults.

**Cause:** The manifest was not generated during setup, or was accidentally deleted.

**Fix:** Run `/arscontexta:reseed` to regenerate the manifest from arscontexta/ops/derivation.md and current configuration.
...
If arscontexta/ops/queue/queue.json is corrupted (invalid JSON, lost entries):

```bash
# Check git history for last valid version
git log --oneline -10 -- arscontexta/ops/queue/queue.json

# Restore from git
git show HEAD~1:arscontexta/ops/queue/queue.json > arscontexta/ops/queue/queue.json.bak
...
mkdir -p arscontexta/ops/archive/$(date +%Y%m%d)
cp arscontexta/ops/queue/queue.json arscontexta/ops/archive/$(date +%Y%m%d)/
cp -r arscontexta/ops/observations/ arscontexta/ops/archive/$(date +%Y%m%d)/

# Reset queue (empty but valid JSON)
echo '{"tasks": []}' > arscontexta/ops/queue/queue.json
...
- Read the relevant skill's SKILL.md file in `arscontexta/skills/arscontexta-[name]/SKILL.md` for full implementation details
