...
2. **Validate** -- schema compliance against arscontexta/templates/note.md. Required fields (description, topics), enum values (status: current|outdated|speculative), constraints.
...
```
1. Read arscontexta/self/identity.md     -- who you are in this project
2. Read arscontexta/self/methodology.md  -- how you process knowledge
3. Read arscontexta/self/goals.md        -- current active threads
4. Check arscontexta/ops/reminders.md    -- time-bound commitments
...
- Process items from arscontexta/inbox/ through the pipeline
...
Session capture hooks save the transcript to arscontexta/ops/sessions/ automatically.
...
The processing queue lives at `arscontexta/ops/queue/queue.json`. Each entry tracks one note through the pipeline:
...
Configured in arscontexta/ops/config.yaml under `processing.chaining`:
...
1. Copy the discussion to `arscontexta/inbox/approval-gate-discussion.md`
2. `/arscontexta-seed arscontexta/inbox/approval-gate-discussion.md`
...
1. `/arscontexta-learn "LLM framework tool authorization patterns"` (researches topic, files to arscontexta/inbox/)
2. `/arscontexta-pipeline arscontexta/inbox/[research-output].md` (full pipeline)
