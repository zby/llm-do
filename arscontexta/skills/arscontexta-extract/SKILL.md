...
1. **`arscontexta/ops/derivation-manifest.md`** — vocabulary mapping, extraction categories, platform hints
...
2. **`arscontexta/ops/config.yaml`** — processing depth, pipeline chaining, selectivity
   - `processing.depth`: deep | standard | quick
   - `processing.chaining`: manual | suggested | automatic
   - `processing.extraction.selectivity`: strict | moderate | permissive

3. **`arscontexta/ops/queue/queue.json`** — current task queue (for handoff mode)
...
- notes folder: `notes/`
- inbox folder: `arscontexta/inbox/`
...
- Completely off-topic (unrelated to llm-do)
...
- If target is empty: scan arscontexta/inbox/ for unprocessed items, pick one
...
4. For each candidate:
   - Use `mcp__qmd__vsearch` with query "[claim as sentence]", collection="docs-notes", limit=5 to check for duplicates
...
| Observation | Action |
|-------------|--------|
| Any observation | Create atomic note in `arscontexta/ops/observations/` with prose-sentence title |
| Tension: content contradicts existing note | Create atomic note in `arscontexta/ops/tensions/` with prose-sentence title |
...
Extract composable notes from source material into docs/notes/.
...
- BETTER thinking: "Does this serve llm-do?"
- BEST thinking: **"Would a future session benefit from this reasoning being a retrievable note?"**
...
**THE RULE:** Implementation without articulation is incomplete. If we DO something but lack a note explaining WHY it works, that articulation needs extraction.
...
See arscontexta/ops/derivation-manifest.md extraction_categories section
...
|----------|--------------|-------------|----------------|
| Core domain notes | Direct assertions about llm-do | note | NO |
| Patterns | Recurring structures across sources | note | NO |
| Comparisons | How different approaches compare, X vs Y, trade-offs | note | NO |
...
| Off-topic general content | Insight unrelated to llm-do | apply selectivity gate | YES |
...
- Named methods: any named system, technique, or framework relevant to llm-do
...
For EVERY candidate, ask: **"Does this serve llm-do?"**
...
**CRITICAL:** This gate exists to filter OUT content that does not serve llm-do. It applies ONLY to standard claims from GENERAL (off-topic) sources.
...
Not already captured in the vault. Semantic duplicate check AND existing notes scan both clear.
...
|----------|-----------------|----------|
| Core domain note | Direct assertion about llm-do | -> note (SKIP selectivity gate) |
...
| Near-duplicate | Semantic search finds related vault note | -> evaluate for enrichment task |
| Off-topic claim | General insight not about llm-do | -> apply selectivity gate |
...
```
mcp__qmd__vsearch  query="[proposed claim as sentence]"  collection="docs-notes"  limit=5
```
...
3. Ask: **"What does source add that existing note lacks?"**
...
ENRICHMENT TASKS (update existing notes):
...
**The enrichment default:** When in doubt between "new note" and "enrichment to existing note", lean toward enrichment. The existing note already has connections, index placement, and integration. Adding to it compounds existing value.
...
Processing selectivity adapts based on `arscontexta/ops/config.yaml`:
...
- index notes (sections: Synthesis, Core Ideas, Tensions, Explorations Needed, Agent Notes)
...
- The chain: research query -> inbox file -> /extract -> docs/notes
...
**Source:** 300-line research document directly relevant to llm-do
...
**Source:** 100-line article with partial relevance to llm-do
...
**The principle:** the goal is to capture everything relevant to llm-do. For domain-relevant sources, that is MOST of the content. The selectivity gate exists for OFF-TOPIC filtering, not for rejecting on-mission content that happens to have a different form.
...
After extraction, for EACH claim, create a task file in `arscontexta/ops/queue/`:
...
## /review
(to be filled by /review phase)
```

### Enrichment Task Files (REQUIRED in handoff mode)

For each ENRICHMENT detected, create a task file in `arscontexta/ops/queue/`:
...
After creating task files, update `arscontexta/ops/queue/queue.json`:
...
Files Modified:
- arscontexta/ops/queue/{source}-NNN.md (claim files)
- arscontexta/ops/queue/{source}-EEE.md (enrichment files, if any)
- arscontexta/ops/queue/queue.json (N claim tasks + M enrichment tasks, 1 entry each)
...
jq '(.tasks[] | select(.id=="TASK_ID")).status = "done" | (.tasks[] | select(.id=="TASK_ID")).completed = "'"$TIMESTAMP"'"' arscontexta/ops/queue/queue.json > tmp.json && mv tmp.json arscontexta/ops/queue/queue.json
```
...
| note just created | //connect | New notes need connections |
| After connecting | //revisit | Old notes need updating |
| Quality check | //review | Combined verification gate |
| System health | /health | Systematic diagnostics |

## Pipeline Chaining

After extraction completes, output the next step based on `arscontexta/ops/config.yaml` pipeline chaining mode:

- **manual:** Output "Next: /connect [created notes]" — user decides when to proceed
- **suggested:** Output next step AND add each created note to `arscontexta/ops/queue/queue.json` with `current_phase: "create"` and `completed_phases: []`
