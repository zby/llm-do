...
2. **`arscontexta/ops/config.yaml`** — for graph thresholds (MOC size limits, orphan thresholds)
...
If graph helper scripts exist in `arscontexta/ops/scripts/graph/`, use them instead of inline analysis:
- `arscontexta/ops/scripts/graph/link-density.sh` for density metrics
- `arscontexta/ops/scripts/graph/orphan-notes.sh` for orphan detection
- `arscontexta/ops/scripts/graph/dangling-links.sh` for dangling link detection
...
If `arscontexta/ops/scripts/graph/find-triangles.sh` exists, use it directly.
...
If `arscontexta/ops/scripts/graph/find-bridges.sh` exists, use it directly.
...
If `arscontexta/ops/scripts/graph/find-clusters.sh` exists, use it directly.
...
If `arscontexta/ops/scripts/graph/influence-flow.sh` exists, use it directly.
...
If `arscontexta/ops/scripts/graph/topic-siblings.sh` exists, use it with the topic argument.
...
If `arscontexta/ops/scripts/graph/n-hop-forward.sh` exists, use it with the note and depth arguments.
...
If `arscontexta/ops/scripts/graph/recursive-backlinks.sh` exists, use it with the note and depth arguments.
...
If `arscontexta/ops/scripts/graph/` does not exist or individual scripts are missing, implement the analysis inline using grep, file reads, and bash loops as shown in each operation's steps. The inline implementations are complete — scripts are optimization, not requirements.
