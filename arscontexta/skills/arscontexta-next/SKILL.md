...
Read `arscontexta/ops/derivation-manifest.md` (or fall back to `arscontexta/ops/derivation.md`) for domain vocabulary mapping. All output must use domain-native terms. If neither file exists, use universal terms (notes, inbox, topic map, etc).
...
**Read queue file** (`arscontexta/ops/queue/queue.json` or `arscontexta/ops/queue.yaml`). If `schema_version` < 3, migrate:
...
|-----------|------------------|
| orphan_notes | For each note in docs/notes/, count incoming [[links]]. Zero = orphan. |
| dangling_links | Extract all [[links]], verify targets exist as files. Missing = dangling. |
| inbox_pressure | Count *.md in arscontexta/inbox/. |
| observation_accumulation | Count status: pending in arscontexta/ops/observations/. |
| tension_accumulation | Count status: pending or open in arscontexta/ops/tensions/. |
| pipeline_stalled | Queue tasks with status: pending unchanged across sessions. |
| unprocessed_sessions | Count files in arscontexta/ops/sessions/ without mined: true. |
...
| methodology_drift | Compare config.yaml modification time vs newest arscontexta/ops/methodology/ note modification time. If config is newer, methodology may be stale. |
...
MAINT_MAX=$(jq '[.tasks[] | select(.id | startswith("maint-")) | .id | ltrimstr("maint-") | tonumber] | max // 0' arscontexta/ops/queue/queue.json)
...
|--------|--------------|----------------|
| **Task stack** | Read `arscontexta/ops/tasks.md` — current priorities and open items | Top items, open count, any deadlines |
| **Queue state** | Read `arscontexta/ops/queue.yaml` or `arscontexta/ops/queue/queue.json` — pending pipeline tasks | Total pending, by phase (create, reflect, reweave, verify), blocked phases |
| **Inbox pressure** | Count `*.md` files in arscontexta/inbox/, find oldest by mtime | Count per subdirectory, age of oldest item in days |
| **Note count** | Count `*.md` in docs/notes/ | Total notes for context |
...
| **Goals** | Read `arscontexta/self/goals.md` or `arscontexta/ops/goals.md` — current priorities, active threads | Priority list, active research directions |
| **Observations** | Count files with `status: pending` in `arscontexta/ops/observations/` | Count |
| **Tensions** | Count files with `status: pending` or `status: open` in `arscontexta/ops/tensions/` | Count |
| **Methodology** | Check `arscontexta/ops/methodology/` for recent captures (files modified in last 7 days) | Count of recent, total count |
| **Health** | Read most recent report in `arscontexta/ops/health/` — note timestamp and issues | Last run date, issue count, any critical issues |
| **Sessions** | Check `arscontexta/ops/sessions/` for files without `mined: true` in frontmatter | Count of unmined sessions |
| **Recent /next** | Read `arscontexta/ops/next-log.md` (if exists) — last 3 recommendations | Previous suggestions to avoid repetition |

**Adaptation rules:**
- Directory names adapt to domain vocabulary (e.g., inbox instead of hardcoded "inbox")
- Skip checks silently for directories that do not exist — do not report "arscontexta/ops/sessions/ not found"
...
INBOX_COUNT=$(find arscontexta/inbox/ -name "*.md" -maxdepth 2 2>/dev/null | wc -l | tr -d ' ')
OLDEST_INBOX=$(find arscontexta/inbox/ -name "*.md" -maxdepth 2 -exec stat -f "%m %N" {} \; 2>/dev/null | sort -n | head -1)

# Note count
NOTE_COUNT=$(ls -1 docs/notes/*.md 2>/dev/null | wc -l | tr -d ' ')

# Pending observations
OBS_COUNT=$(grep -rl '^status: pending' arscontexta/ops/observations/ 2>/dev/null | wc -l | tr -d ' ')

# Pending tensions
TENSION_COUNT=$(grep -rl '^status: pending\|^status: open' arscontexta/ops/tensions/ 2>/dev/null | wc -l | tr -d ' ')

# Unmined sessions
SESSION_COUNT=$(grep -rL '^mined: true' arscontexta/ops/sessions/*.md 2>/dev/null | wc -l | tr -d ' ')
```
...
| **Slow** | Health check not run in 14+ days, index oversized (>40 notes), link density below 2.0 average, low note count relative to time | Background — annoying but not blocking | These are maintenance tasks. Important for long-term health but not urgent. |
...
If `arscontexta/ops/tasks.md` has open items, recommend from the task stack. User-set priorities override all automated recommendations because:
...
| Inbox > 5 items | /extract [specific file] | "Your inbox has [N] items (oldest: [age]). [File X] has the highest connection potential based on [reason]." |
...
| Signal | Recommendation | Rationale Template |
|--------|---------------|-------------------|
| Queue backlog > 10 | /ralph [N] | "[N] pipeline tasks are pending. Your newest docs/notes lack connections, which means they can't participate in synthesis." |
...
| Low link density | /revisit on lowest-density note | "Your graph has an average link density of [N]. Reweaving sparse notes increases traversal paths." |
...
next
  ...
  or reweave older docs/notes to deepen the graph.
```
...
Read `arscontexta/ops/next-log.md` (if it exists). Check the last 3 entries.
...
| Good | Bad |
|------|-----|
| `/extract arscontexta/inbox/article-on-spaced-repetition.md` | "process some inbox items" |
...
Append to `arscontexta/ops/next-log.md` (create if missing):
...
```
  ...
  - Reweaving older docs/notes to deepen connections
  - Reviewing and updating goals.md itself
```

### No Goals File

Recommend creating `arscontexta/self/goals.md` or `arscontexta/ops/goals.md` first. Without priorities, recommendations lack grounding and the system cannot distinguish between "important to the user" and "detected by automation."

```
  Recommended: Create arscontexta/ops/goals.md
...
If `arscontexta/ops/next-log.md` has not been updated in 14+ days, the user may not be running /next regularly. Note this but do not make it a recommendation — /next is optional, not mandatory.
...
| Ignoring task stack | User-set priorities exist for a reason | Always check arscontexta/ops/tasks.md first |
