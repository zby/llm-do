...
1. **`arscontexta/ops/derivation-manifest.md`** — vocabulary mapping, dimension positions, platform hints
   - Use `vocabulary.notes` for the notes folder name
   - Use `vocabulary.note` / `vocabulary.note_plural` for note type references
   - Use `vocabulary.topic_map` / `vocabulary.topic_map_plural` for MOC references
   - Use `vocabulary.inbox` for the inbox folder name

2. **`arscontexta/ops/config.yaml`** — current live configuration (the "after" state)

3. **`arscontexta/ops/derivation.md`** — original derivation record (the "before" state)

If these files don't exist, report error: "Cannot refactor without both arscontexta/ops/config.yaml and arscontexta/ops/derivation.md. These files establish the baseline and current configuration."
...
Compare each dimension in `arscontexta/ops/config.yaml` against the same dimension in `arscontexta/ops/derivation.md`:

**Step 1: Read both files**

Read `arscontexta/ops/config.yaml` and `arscontexta/ops/derivation.md` fully. Extract the position for each of the 8 dimensions from both.
...
--=={ refactor }==--
  ...
  or edit arscontexta/ops/config.yaml directly, then run /refactor again.
```
...
|--------|-------------------|-------------|
| **Granularity shift** | Note templates, extraction depth in /extract, processing skills, context file "Note Design" section | Template body length guidance, extraction granularity settings, composability test thresholds |
...
| **Processing shift** | /extract depth settings, /connect pass count, pipeline skills, context file "Processing Pipeline" section, config.yaml processing.depth | Extraction thoroughness, connection evaluation depth, chaining mode |
...
- Count affected notes: `grep -rl '[old value]' docs/notes/*.md | wc -l`
...
Detected [N] dimension changes:
  ...
  [dimension]: [old] -> [new]
    ...
    Content impact: [N] existing notes need [what]
    Risk: [low/medium/high]
...
echo "## $DATE: /refactor — [dimensions changed]" >> arscontexta/ops/changelog.md
echo "" >> arscontexta/ops/changelog.md
echo "Changes planned:" >> arscontexta/ops/changelog.md
echo "[list of changes]" >> arscontexta/ops/changelog.md
```
...
|-----------------|---------------------|
| Granularity | /extract, /review, /validate |
| Processing | /extract, /connect, /revisit, /review, /ralph, /pipeline |
| Linking | /connect, /revisit |
| Navigation | /connect (MOC update logic) |
...
For schema changes:
1. Read affected templates from arscontexta/ops/arscontexta/templates/ (or arscontexta/templates/)
...
1. Update `arscontexta/ops/derivation.md` with:
   - New dimension positions
   - Rationale for each change
   - Date of change
   - Reference to /architect recommendation (if applicable)

2. Update `arscontexta/ops/derivation-manifest.md` to sync machine-readable config

3. Update `arscontexta/ops/config.yaml` to confirm it matches derivation (should already match since config triggered the refactor)
...
3. **Link updates:** Update all wiki links affected by renames
   ...
   arscontexta/ops/scripts/rename-note.sh "old name" "new name"
   ```
...
--=={ refactor complete }==--
  ...
  Content migration:
    [N] notes updated with new schema fields
    [N] wiki links updated
    [N] indexes restructured

  Kernel validation: [N]/[N] checks PASS
  [Any warnings or issues]

  Derivation updated: arscontexta/ops/derivation.md
  Changelog updated: arscontexta/ops/changelog.md
```
...
### No arscontexta/ops/config.yaml or No arscontexta/ops/derivation.md

Cannot refactor without both files. Report error and suggest creating them:
```
Cannot run /refactor: [missing file] not found.
  - arscontexta/ops/config.yaml is the live configuration
  - arscontexta/ops/derivation.md is the original design baseline
...
- Creating new directories (arscontexta/self/ for self_space on)
...
| self_space | Archive arscontexta/self/ contents, remove arscontexta/self/ references | Create arscontexta/self/ structure, generate identity files |
