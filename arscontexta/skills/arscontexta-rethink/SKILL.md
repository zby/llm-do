...
2. **`arscontexta/ops/config.yaml`** — thresholds, processing preferences
...
3. **`arscontexta/ops/methodology/`** — existing methodology notes (read all to understand current system self-knowledge)
...
Every rule in the context file, every workflow in a skill, every assumption baked into the architecture was a hypothesis at some point. Hypotheses need testing against reality. Observation notes in `arscontexta/ops/observations/` capture friction from actual use. Tension notes in `arscontexta/ops/tensions/` capture unresolved conflicts. Rethink first triages these individually (some become docs/notes, some become methodology updates, some get archived), then compares remaining evidence against what the system assumes and proposes changes when patterns emerge.
...
Rule Zero: arscontexta/ops/methodology/ is the canonical specification of how this system operates. Before triaging observations, check whether the system has drifted from what the methodology says it should do.

### 0a. Load Methodology State

```bash
# Get all methodology notes with their metadata
for f in arscontexta/ops/methodology/*.md; do
  echo "=== $f ==="
...
Read:
- `arscontexta/ops/config.yaml` — current configuration state
- The context file (CLAUDE.md) — current behavioral instructions
- `arscontexta/ops/derivation-manifest.md` — vocabulary and feature state
...
CONFIG_MTIME=$(stat -f %m arscontexta/ops/config.yaml 2>/dev/null || stat -c %Y arscontexta/ops/config.yaml 2>/dev/null || echo 0)
NEWEST_METH=$(ls -t arscontexta/ops/methodology/*.md 2>/dev/null | head -1)
...
For each drift finding, create an observation note in `arscontexta/ops/observations/`:
...
```bash
OBS_PENDING=$(grep -rl '^status: pending' arscontexta/ops/observations/ 2>/dev/null)
OBS_COUNT=$(echo "$OBS_PENDING" | grep -c . 2>/dev/null || echo 0)
TENSION_PENDING=$(grep -rl '^status: pending\|^status: open' arscontexta/ops/tensions/ 2>/dev/null)
...
Also read `arscontexta/ops/methodology/` to understand existing methodology notes — this prevents creating duplicates and informs whether new observations should extend existing methodology rather than create new notes.
...
|-------------|---------|---------------|--------|
| PROMOTE | Reusable insight worth keeping as a permanent note | General principle across sessions. Would work as a claim note. Crystallized insight, not operational guidance. | Create note in docs/notes/, set observation `status: promoted`, add `promoted_to: [[title]]` |
...
- Tension reveals a genuine conflict between two docs/notes → PROMOTE (create a tension note or resolution note)
...
--=={ rethink — Triage }==--
  ...
  METHODOLOGY ([count])
    [filename] — [title] → create/update arscontexta/ops/methodology/[name].md
    [filename] — [title] → extends existing arscontexta/ops/methodology/[name].md
...
**For PROMOTE items:**
1. Create note with prose-as-title in docs/notes/
2. Follow standard note schema: YAML frontmatter (description, type, created), body developing the insight, Topics footer linking to relevant index(s)
...
**Update MOCs:** After triage execution, update `arscontexta/ops/observations.md` and `arscontexta/ops/tensions.md` to reflect status changes. Move entries between Pending/Promoted/Archived/Resolved/Dissolved sections as appropriate.

---

## Phase 2: Methodology Folder Updates

For items triaged as METHODOLOGY, create or update notes in `arscontexta/ops/methodology/`.
...
5. Update the observation: set `status: implemented`, add `implemented_in: arscontexta/ops/methodology/[existing-file]`

### Checking for Methodology Duplicates

Before creating a new methodology note:
1. Read all files in `arscontexta/ops/methodology/` (these are small)
...
After creating or updating methodology notes, update `arscontexta/ops/methodology.md`:
...
4. **Methodology notes** — patterns in `arscontexta/ops/methodology/` by category
...
| Methodology convergence | Multiple /remember captures in arscontexta/ops/methodology/ pointing at the same behavioral pattern | Methodology note needs elevation to context file | A methodology learning has been validated enough to become a system-level rule |
...
5. **Compare methodology notes against context file:** If `arscontexta/ops/methodology/` has 3+ notes in the same category that are not reflected in the context file, the methodology has converged enough for elevation.
...
1. **Specific file references** — not "update the context file" but "update arscontexta/ops/context.md, section 'Processing Pipeline', paragraph 3"
...
4. **Log to arscontexta/ops/changelog.md** (create if missing):
...
If any observations were promoted to docs/notes:

```
  [count] docs/notes were promoted from observations.
...
- Add each promoted note to the queue with `current_phase: "reflect"` (the note already exists, so skip create)
- Report queue additions

### Session Log

After rethink completes, capture the session itself. Create or append to `arscontexta/ops/rethink-log.md`:
...
### No arscontexta/ops/observations/ or arscontexta/ops/tensions/
...
2. Recommend creating them: "The operational learning loop requires `arscontexta/ops/observations/` and `arscontexta/ops/tensions/`. Create these directories and their MOC files to begin capturing system friction."
...
- Create docs/notes directly from observations without going through standard pipeline (PROMOTE adds to queue)
...
- Log changes to arscontexta/ops/changelog.md for evolution tracking
