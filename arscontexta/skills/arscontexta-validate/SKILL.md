...
2. **`arscontexta/ops/config.yaml`** — processing depth
...
- If target is "all" or "notes": validate all notes in docs/notes/ directory
...
1. Check the note's location — notes in docs/notes/ use the standard note template
2. Check the `type` field in frontmatter — specialized types may have dedicated templates
3. Look for a templates directory (check `arscontexta/ops/arscontexta/templates/` or domain-specific path from derivation manifest)
...
1. Discover all .md files in docs/notes/ directory
2. Optionally include additional directories (e.g., arscontexta/self/memory/) if they exist
