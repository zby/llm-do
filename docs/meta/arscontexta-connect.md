...
_config:
  ...
  placeholders:
    ...
    next_phase: "vocabulary.reweave"
    cmd_next_phase: "vocabulary.cmd_reweave"
    queue_file: "arscontexta/ops/queue/queue.json"
    lock_dir: "arscontexta/ops/queue/.locks/qmd.lock"
    qmd_collection: "semantic-search.collections (notes key)"
    depth: "processing.depth"
    chaining: "processing.chaining"
...
|-------|-------------------|
| deep | Full dual discovery (index + semantic search). Evaluate every candidate. Multiple passes. Synthesis opportunity detection. Bidirectional link evaluation for all connections. |
...
8. If task file in context: update the {{ phase_name }} section
...
Before using semantic search, verify the index is current. This is self-healing: if notes were created outside the pipeline (manual edits, other skills), {{ phase_name }} catches the drift before searching.
...
2. **If MCP unavailable** (tool fails or returns error): fall back to bash:
   ...
   qmd_count=$(qmd status 2>/dev/null | grep -A2 '{{ qmd_collection }}' | grep 'documents' | grep -oE '[0-9]+' | head -1)
   rm -rf "$LOCKDIR"
   ```
3. Count actual files:
   ```bash
   file_count=$(ls -1 {{ notes_path }}/*.md 2>/dev/null | wc -l | tr -d ' ')
   ```
...
LOCKDIR="{{ lock_dir }}"
while ! mkdir "$LOCKDIR" 2>/dev/null; do sleep 2; done
qmd query "[note's core concepts]" --collection {{ qmd_collection }} --limit 15 2>/dev/null
rm -rf "$LOCKDIR"
```
...
When you edit an older note to add a reverse link, you MAY flag it for full reconsideration via {{ next_phase }}. But SKIP {{ next_phase }} flagging if ANY of these apply:
...
| Note was processed in current batch | Do not re-process what was just processed |
| Note is an index | indexes are navigation, not claims to reconsider |
...
**index exploration:**
- Read [index-name](./index-name.md) — found candidates: [note A](./note-a.md), [note B](./note-b.md), [note C](./note-c.md)
...
```
=== RALPH HANDOFF: {{ phase_name }} ===
Target: [note name](./note-name.md)

Work Done:
- Discovery: index [index-name](./index-name.md), query "[query]" (MCP|bash|grep-only), grep "[term]"
- Connections added: N (articulation test: PASS)
- index updates: [index-name](./index-name.md) Core Ideas section
- Synthesis opportunities: [count or NONE]

Files Modified:
- {{ notes_path }}/[note name].md (inline links added)
- {{ notes_path }}/[index-name].md (Core Ideas updated)
- [task file path] ({{ phase_name }} section)
...
Queue Updates:
- Advance phase: {{ phase_name }} -> {{ next_phase }}
...
When running in handoff mode via /ralph, the prompt includes the task file path. After completing the workflow, update the `## {{ phase_name }}` section of that task file with:
- Connections added and why
- index updates made
- Articulation test results
- Discovery trace summary

**Critical:** The handoff block is OUTPUT, not a replacement for the workflow. Do the full workflow first, update task file, then format results as handoff.
...
jq '(.tasks[] | select(.id=="TASK_ID")).current_phase = "{{ next_phase }}" |
    (.tasks[] | select(.id=="TASK_ID")).completed_phases += ["{{ phase_name }}"]' \
...
**Active chaining: {{ chaining }}**

After connection finding completes, output the next step based on the active chaining mode:

- **manual:** Output "Next: {{ cmd_next_phase }} [note]" — user decides when to proceed
- **suggested:** Output next step AND advance task queue entry to `current_phase: "{{ next_phase }}"`
