...
## Findings
- **EntryFunction tool calls now follow approval policy:** entry code is trusted, but tool calls are wrapped like workers for parity and observability. Keep documentation aligned with this behavior.
...
## Conclusion
Ctx runtime is stable; remaining concerns are schema-ref re-exec behavior and max-depth error context. Entry tool-plane parity is aligned; keep docs in sync as the runtime evolves.
