...
- Path handling is duplicated across `_split_function_ref()`, `_is_path_ref()`,
  ...
  `path.py:function` references could remove branching. Done: shared helpers
  moved to `runtime/path_refs.py`.
...
- Relative-path resolution logic mirrors `input_model_refs`. Consider sharing a
  common path-ref helper to reduce duplicate rules. Done: shared helper
  introduced in `runtime/path_refs.py`.
