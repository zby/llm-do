...
- **Snapshot/restore:** The sub-agent gets a copy of the browser state, operates on it independently, and the parent's state is untouched. Expensive but correct for isolation.
...
|--------|-------------|---------|
| **Global singleton** | Expensive shared resource, concurrent-safe | MCP servers, connection pools |
| **Per-run fresh** | State should not leak between conversations | Browser sessions, caches, counters |
| **Per-agent-call fresh** | Sub-agents should not interfere with parent state | Browser sessions in delegation scenarios |
| **Inherited from parent** | Sub-agents should share parent's context | Database transactions, rate limiters |

PydanticAI currently offers two: global (static toolset) and per-run (DynamicToolset with a well-behaved factory). llm-do adds per-agent-call via `CallScope`. Nobody offers "inherited from parent" as a first-class mechanism — it requires explicit plumbing through dependencies.

A traits system that doesn't model these lifecycle policies will inherit the same ambiguity. `Trait.get_toolset(ctx: RunContext)` could return any of these — but the framework can't validate or enforce the lifecycle contract because it doesn't know which policy the trait intends.
