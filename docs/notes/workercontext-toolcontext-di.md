...
## Findings
- The runtime context is `llm_do.ctx_runtime.Context`.
- Tools receive a `RunContext[Context]` from pydantic-ai. The runtime context is
...
- There is no separate `@tool_context` decorator; context access is opt-in by
  accepting `RunContext[Context]` as the first parameter.
...
- Do we want a small Protocol ("ToolContext") to type the minimal surface
  (for example, just `call(...)`) instead of exposing the full `Context`?

## Conclusion
Context injection is handled by pydantic-ai via `RunContext[Context]`. Tools opt
