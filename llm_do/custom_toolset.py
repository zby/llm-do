...
This module provides CustomApprovalToolset which:
...
class CustomToolsetInner(AbstractToolset[WorkerContext]):
    """Core custom tools toolset (no approval logic).

    Loads and exposes custom tools from a Python module.
    Approval logic is handled by the CustomApprovalToolset wrapper.
    """
    ...
    def _load_module(self) -> None:
        ...
        if not self.tools_path.exists():
            logger.warning(f"Custom tools path does not exist: {self.tools_path}")
            return

        # Load the module from the file path
        spec = importlib.util.spec_from_file_location(
            f"{self.worker_name}_tools", self.tools_path
        )
        if spec is None or spec.loader is None:
            logger.warning(f"Could not load custom tools from {self.tools_path}")
            return
        ...
        try:
            ...
            logger.error(f"Error loading custom tools from {self.tools_path}: {e}")
            return
        ...
        for tool_name in self.allowed_tools:
            if not hasattr(module, tool_name):
                logger.warning(f"Custom tool '{tool_name}' not found in {self.tools_path}")
                continue
            ...
            if not (
                ...
                logger.warning(f"Custom tool '{tool_name}' is not a function in {self.tools_path}")
                continue
