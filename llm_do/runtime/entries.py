...
class _MessageLogList(list):
    ...
    def __init__(self, runtime: CallRuntimeProtocol, worker_name: str, depth: int) -> None:
        super().__init__()
...
def _capture_message_log(
    runtime: CallRuntimeProtocol, *, worker_name: str, depth: int
) -> Any:
...
class _DefaultEntryToolSchema(BaseModel):
    """Default schema for agent entries exposed as tools."""
...
def build_entry_tool(entry: "AgentEntry", toolset: AbstractToolset[Any]) -> ToolsetTool[Any]:
    """Build a ToolsetTool for exposing an AgentEntry as a callable tool."""
    desc = entry.description or entry.instructions
    desc = desc[:200] + "..." if len(desc) > 200 else desc
    schema = entry.schema_in or _DefaultEntryToolSchema
    return ToolsetTool(
        toolset=toolset,
        tool_def=ToolDefinition(name=entry.name, description=desc, parameters_json_schema=schema.model_json_schema()),
        max_retries=0,
...
class EntryToolset(AbstractToolset[Any]):
    """Adapter that exposes an AgentEntry as a single tool for another agent."""
    entry: "AgentEntry"
...
class EntryFunction:
    ...
    def start(
        ...
        *,
        ...
        call_runtime: CallRuntimeProtocol
        if parent is None:
            ...
            call_runtime = parent.spawn_child(
                active_toolsets=wrapped_toolsets,
                model=NULL_MODEL,
                invocation_name=self.name,
            )
...
@dataclass
class AgentEntry:
    """An LLM-powered entry backed by a PydanticAI Agent."""
    name: str
    instructions: str
    ...
    def __post_init__(self, compatible_models: list[str] | None) -> None:
        ...
        for spec in self.toolset_specs:
            if not isinstance(spec, ToolsetSpec):
                raise TypeError("AgentEntry toolset_specs must contain ToolsetSpec instances.")
    ...
    def start(
        ...
        *,
        ...
        """Start a call scope for this entry."""
        resolved_model = self.model
        if resolved_model is None:
            raise RuntimeError("AgentEntry model is not set")
    ...
    async def call(
        ...
        scope = self.start(run_ctx.deps.runtime, parent=run_ctx.deps)
        try:
    ...
    async def _run_with_event_stream(
        self,
        agent: Agent[CallRuntimeProtocol, Any],
