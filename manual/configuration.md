---
description: Guide to ops/config.yaml -- system dimensions, processing depth, feature flags, and tuning for llm-do development
type: manual
...
The knowledge system's behavior is controlled by `ops/config.yaml`. This file is the live configuration that all skills read at runtime. Changes take effect immediately on the next skill invocation.

For the reasoning behind each configuration choice, see `ops/derivation.md`. For the machine-readable manifest used by skills at runtime, see `ops/derivation-manifest.md`.
...
Dimensions are the system's structural parameters. They were set during `/arscontexta:setup` based on the llm-do project's characteristics. To change a dimension, edit ops/config.yaml and run `/arscontexta-refactor` to plan the restructuring.
...
To add a category, append it to this list and update `ops/derivation-manifest.md` with the new category's `what_to_find` and `output_type`.
...
1. Set `features.semantic-search: true` in ops/config.yaml
...
1. Edit `ops/config.yaml` directly
...
**Important:** Dimension changes may require restructuring notes, indexes, or templates. Always run `/arscontexta-refactor` after changing a dimension -- it compares the new config against ops/derivation.md and generates a migration plan.
