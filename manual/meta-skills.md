...
/arscontexta-remember -- capture the friction signal
    |
    v
Accumulate observations (ops/observations/)
    |
    v
...
**Purpose:** Capture operational friction as methodology notes in ops/observations/ or ops/methodology/.
...
This creates an observation note in `ops/observations/` with a prose-sentence title:
...
Scan ops/sessions/ transcript archives for recurring patterns.
...
- If `ops/observations/` has 10+ pending items: suggests running `/arscontexta-rethink`
- If `ops/tensions/` has 5+ pending items: suggests running `/arscontexta-rethink`

These thresholds are configurable in ops/config.yaml.
...
Compares ops/config.yaml against ops/derivation.md to detect configuration drift. If dimensions have shifted without a corresponding derivation update, flags the drift.
...
- Creates or updates methodology notes in ops/methodology/
...
- Applied directly (configuration changes via ops/config.yaml edits)
- Routed to `/arscontexta:architect` (structural changes requiring schema or template modification)
- Deferred (logged as future work in self/goals.md)
...
1. **Local methodology** (ops/methodology/) -- "How does MY system work?" Notes about this specific knowledge system's configuration, learned behaviors, and operational patterns.
...
```
Checks ops/derivation.md for the conversation signals, then consults research on granularity trade-offs.
...
| Situation | Approach |
|-----------|----------|
| Quick factual lookup | Read ops/config.yaml or ops/derivation.md directly |
...
- Configuration changes (edit ops/config.yaml directly, then `/arscontexta-refactor`)
...
| Rethink proposes a config change | Edit ops/config.yaml + `/arscontexta-refactor` |
