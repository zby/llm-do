...
- [Generate LLM instructions at build time, don't parameterise them](./generate-instructions-at-build-time.md) *(note)* — LLM instructions with variable placeholders add per-invocation interpretation cost — generate concrete instructions at build time instead, paying the flexibility cost once
