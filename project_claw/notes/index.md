...
- [Extracting the claw system requires a two-tier distillation model](./extract-kb-as-standalone-project.md) *(note)* — Extraction plan for the claw system as a standalone repo with two tiers — distillate-only (skills, templates, scripts) for operational use, and distillate+methodology for understanding, adaptation, and skill maintenance
