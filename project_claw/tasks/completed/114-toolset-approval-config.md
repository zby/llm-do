...
- Outcome:
  ...
  - Filesystem toolsets are `filesystem_cwd`/`filesystem_cwd_ro` and `filesystem_project`/`filesystem_project_ro`.
  - `base_path` is derived from the process CWD for `filesystem_cwd*` and the worker file directory for `filesystem_project*`.
- Follow-ups:
  - Optionally add `@llm_do_tool` helper decorator to set per-tool approval config on FunctionToolset.

## Tasks
- [x] Add built-in toolset entries for `filesystem_cwd*`/`filesystem_project*` and shell profiles with approval config attributes.
...
- [x] Add a read-only filesystem toolset class; set `base_path` from CWD/worker dir; expose only read/list for RO.
- [x] Update docs/examples to the new toolset config model.
- [x] Update tests for toolset loading/approvals changes.
- [x] Update docs/examples/tests to match the new toolset model (`filesystem_cwd*`/`filesystem_project*`).
