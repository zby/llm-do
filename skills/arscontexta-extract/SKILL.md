...
| Tension: content contradicts existing note | Create atomic note in `ops/tensions/` with prose-sentence title |
...
See ops/derivation-manifest.md extraction_categories section
...
- **suggested:** Output next step AND add each created note to `ops/queue/queue.json` with `current_phase: "create"` and `completed_phases: []`
