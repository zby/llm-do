...
## Goal
Run the simplification prompt on `llm_do/ui/events.py` and its local imports, producing a review report with proposed simplifications.

## Context
- Relevant files/symbols:
  - `llm_do/ui/events.py` (primary target)
...
## Tasks
- [ ] Read events.py and identify its local imports
- [ ] Analyze with the simplification prompt above
- [ ] Write review report to `docs/notes/reviews/simplify-ui-events.md`

## Current State
Not started.

## Notes
- Events define the UI event system; simplifications clarify event flow and reduce duplication in event handling
