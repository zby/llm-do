...
## Goal
Run the simplification prompt on `llm_do/config.py` and its local imports, producing a review report with proposed simplifications.

## Context
- Relevant files/symbols:
  - `llm_do/config.py` (primary target)
...
## Tasks
- [ ] Read config.py and identify its local imports
- [ ] Analyze with the simplification prompt above
- [ ] Write review report to `docs/notes/reviews/simplify-config.md`

## Current State
Not started.

## Notes
- Config module handles all configuration loading; simplifications clarify config precedence and reduce parsing complexity
