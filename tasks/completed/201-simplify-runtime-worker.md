...
## Goal
Run the simplification prompt on `llm_do/runtime/worker.py` and its local imports, producing a review report with proposed simplifications.

## Context
- Relevant files/symbols:
  - `llm_do/runtime/worker.py` (primary target)
  - Local imports from this repo (worker_file.py, shared.py, etc.)
- Output:
