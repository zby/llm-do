...
@skip_no_llm
def test_code_analyzer_count_files(code_analyzer_registry, default_model, approve_all_controller):
    ...
    result = asyncio.run(
        run_worker_async(
            registry=code_analyzer_registry,
            worker="code_analyzer",
            input_data="How many Python files are there?",
...
@skip_no_llm
def test_code_analyzer_find_pattern(code_analyzer_registry, default_model, approve_all_controller):
    ...
    result = asyncio.run(
        run_worker_async(
            registry=code_analyzer_registry,
            worker="code_analyzer",
            input_data="Find all TODO comments",
            cli_model=default_model,
            approval_controller=approve_all_controller,
        )
    )
...
@skip_no_llm
def test_code_analyzer_line_count(code_analyzer_registry, default_model, approve_all_controller):
    ...
    result = asyncio.run(
        run_worker_async(
            registry=code_analyzer_registry,
            worker="code_analyzer",
            input_data="Count the lines in lines_test.py",
            cli_model=default_model,
            approval_controller=approve_all_controller,
        )
    )
    ...
    assert any(num in result.output for num in ["5", "five"])
