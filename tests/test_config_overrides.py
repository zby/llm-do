...
class TestParseSetOverride:
    ...
    def test_parse_nested_key(self):
        key, value = parse_set_override("config.retry.timeout=30")
        assert key == "config.retry.timeout"
        assert value == 30
...
class TestApplySetOverride:
    ...
    @pytest.mark.parametrize("initial,key,value,expected", [
        ...
        ({"config": {"retry": {"timeout": 30, "other": "keep"}}},
         "config.retry.rules", [{"pattern": "git"}],
         {"config": {"retry": {"timeout": 30, "other": "keep", "rules": [{"pattern": "git"}]}}}),
        # Bracketed literal key
        ({},
         'config["custom.key"].timeout', False,
         {"config": {"custom.key": {"timeout": False}}}),
    ])
...
class TestApplyOverrides:
    ...
    def test_apply_nested_override(self):
        ...
        result = apply_overrides(
            data,
            set_overrides=["config.retry.timeout=30"]
        )
        assert result["config"]["retry"]["timeout"] == 30
...
class TestIntegration:
    ...
    def test_toolsets_override(self):
        ...
        result = apply_overrides(
            data,
            set_overrides=['config.rules=[{"pattern": "git"}]']
        )
        assert result["config"]["rules"] == [{"pattern": "git"}]

    def test_bracketed_key_override(self):
        """Test overriding bracketed literal keys."""
        data = {"name": "worker"}
        result = apply_overrides(
            data,
            set_overrides=[
        ...
        assert result["config"]["custom.key"]["timeout"] is False
    ...
    def test_complex_nested_override(self):
        ...
        result = apply_overrides(
            data,
            set_overrides=[
                "config.shell.rules=[{\"pattern\": \"git *\"}]",
                "config.shell.timeout=60",
                "config.filesystem.allowed_paths=[\"/tmp\"]",
            ]
        )
        assert result["config"]["shell"]["rules"] == [{"pattern": "git *"}]
        assert result["config"]["shell"]["timeout"] == 60
        assert result["config"]["filesystem"]["allowed_paths"] == ["/tmp"]
